...
//	ENSURE YOU ALREADY HAVE THESE FILEPATHS WITHIN YOUR LIST OF #USING'S AND IF NOT, ADD THEM AT THE BOTTOM OF THE LIST:
...
//	ONLY USE THIS FUNCTION IF YOU CURRENTLY DO NOT HAVE A CALLBACK FUNCTION FOR PLAYER SPAWNING
/*
function __init__()
{
	callback::on_spawned( &on_player_spawned );		// called on player spawn
}

function on_player_spawned(player) // OMIT THIS FUNCTION IF YOU ALREADY HAVE ONE SIMILAR TO IT.
{
	self thread watchformoneyshare(); //	PLACE THIS INSIDE YOUR "ON_PLAYER_SPAWNED" FUNCTION IF YOU ALREADY HAVE ONE
}
*/

//	SOMEWHERE IN YOUR MAP FILE, COPY AND PASTE THIS FUNCTION:
...
//	PLACE THIS FUNCTION DIRECTLY BELOW THE WATHCFORMONEYSHARE() FUNCTION.
...
//	PLACE THIS FUNCTION DIRECTLY BELOW THE STARTMONEYSHARE() FUNCTION.
...
//	PLACE THIS FUNCTION DIRECTLY BELOW THE SHAREDPOINTSVALUE() FUNCTION. THIS ENSURES THE POWERUP IS SPAWNED IN THE PLAYABLE SPACE. COPIED FROM TREYARCH'S BGB SCRIPT.
